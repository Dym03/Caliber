...
CLINVAR_URL = "https://ftp.ncbi.nlm.nih.gov/pub/clinvar/xml/ClinVarVCVRelease_00-latest.xml.gz"
...
def download_monthly_release():
    ...
    if LOCAL_FILE.exists():
        ...
        if verify_md5(LOCAL_FILE, expected_md5):
            logger.info("Your local ClinVar archive is already up to date with the latest monthly release. Skipping.")
            return
        logger.warning("New monthly release detected or local file is corrupted. Starting download...")
    ...
    with urllib.request.urlopen(CLINVAR_URL) as remote_stream:
        with open(LOCAL_FILE, "wb") as local_file:
            while chunk := remote_stream.read(65536):  # Large 64KB buffers for high throughput
    ...
    if verify_md5(LOCAL_FILE, expected_md5):
        ...
        logger.error("CRITICAL: Download completed but MD5 verification failed. The file might be corrupted.")
