...
class Command(BaseCommand):
    ...
    def handle(self, *args: tuple, **options: dict) -> None:
        ...
        try:
            ...
            self.stdout.write(self.style.SUCCESS(f"Successfully sync'd {parsed_counter} variants with core schema."))
            logger.info(f"ClinVar synchronization finished successfully. Processed {parsed_counter} items.")

        except Exception as e:
            # Global execution safety net wrap
            logger.critical(f"Pipeline crashed during streaming ingestion phase: {str(e)}", exc_info=True)
            self.stderr.write(self.style.ERROR(f"Critical execution error: {str(e)}"))
            raise CommandError("Sync script aborted due to unhandled pipeline exception.")
