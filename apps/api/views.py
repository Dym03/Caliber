...
def search_variants(request):
    ...
    if not (variant or gene or dbsnp):
        return JsonResponse({"results": [], "meta": {"has_next": False, "total_count": 0}})
    ...
    if gene:
        gene_id = Gene.objects.filter(symbol__iexact=gene).values_list('id', flat=True).first()
        # Fast-fail: If they searched a gene that doesn't exist, stop immediately
        if not gene_id:
            return JsonResponse({"results": [], "meta": {"has_next": False, "total_count": 0, "total_pages": 0}})
    ...
    if not patient_queryset.exists():
        ...
        target_queryset = (
            ...
            .distinct()
            .order_by("chromosome", "position")  # Consistent order is critical for pagination
        )
    ...
    try:
        ...
        return JsonResponse({"results": [], "meta": {"has_next": False, "total_count": paginator.count}})
